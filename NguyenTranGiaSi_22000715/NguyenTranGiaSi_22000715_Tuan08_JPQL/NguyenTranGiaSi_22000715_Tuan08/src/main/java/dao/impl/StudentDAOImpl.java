...
public class StudentDAOImpl extends GenericDAOImpl<Student, Integer> implements StudentDAO {
    ...
    public List<Student> listStudentsStudyingCourseWithHighestScore(String courseName) {
        Double highestGrade = em.createQuery(
                        "SELECT MAX(sg.grade) " +
                                "FROM StudentGrade sg " +
                                "WHERE sg.course.title = :courseName AND sg.grade IS NOT NULL",
                        Double.class)
        ...
        return em.createQuery(
                        "SELECT sg.student " +
                                ...
                                "AND sg.grade = :highestGrade", Student.class)
                .setParameter("courseName", courseName)
                .setParameter("highestGrade", highestGrade)
                .getResultList();
    }
